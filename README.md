...
In order to view the Events, you need to be in secure mode. All the setup is already configured inside the project.

When you execute `yarn dev`, a custom certificate is generated for your machine and it's saved into your machine (`localhost-key.pem` and `localhost.pem`) at the root folder of the project. You will need to accept this certificate the first time you visit the local website in order to access it. This is completely secure beacuse all this setup is happening in on your machine and nothing is sent over the internet.

This process will happen only once per year (certificates' validity length is 1 year) and the next time you run the command, the previous generated certificates will be used.

This setup is only for devlopment, certificates are generated by GitHub Pages when the website is pushed into production mode.
